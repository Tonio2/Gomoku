...
void test_problems() {
  ...
  while (std::getline(in, line)) {
    ...
    // Suggest a move
    MoveEvaluation moveEvalutation =
        AI.suggest_move(game, moves.size() % 2 == 0 ? X : O);
    // Get the best move
    ...
    std::string best_move_string =
        std::string(1, coordinate_to_char(bestMove.first)) +
        std::string(1, coordinate_to_char(bestMove.second));
    if (std::find(forbidden_moves.begin(), forbidden_moves.end(),
                  best_move_string) != forbidden_moves.end()) {
      // Print the description then print "...NOK" to std::cout in red color
      std::cout << "\033[1;31m" << description << "...NOK"
                << "\033[0m" << std::endl;
    } else if (std::find(expected_moves.begin(), expected_moves.end(),
                         best_move_string) != expected_moves.end()) {
    ...
    std::cout << "Total move evaluated count: " << AI.move_evaluated_count
              << std::endl;
    std::cout << "Total evaluation needed count: " << AI.evaluation_needed_count
              << std::endl;
    std::cout << "Percentage of moves evaluated: "
              << (AI.move_evaluated_count * 100) / AI.move_count << "%"
              << std::endl;
    std::cout << "Percentage of evaluations needed: "
              << (AI.evaluation_needed_count * 100) / AI.move_evaluated_count
              << "%" << std::endl;
  ...
  std::cout << "Average time: " << totalTime / problemCount << " ms"
            << std::endl;
}
...
void test_problem(int problem_idx) {
  ...
  std::cout << "Best move for player " << player << ": "
            << coordinate_to_char(bestMove.first)
            << coordinate_to_char(bestMove.second) << std::endl;
...
void test_eval(std::string moves_string) {
  ...
  std::cout << "Scores: X:" << game.get_player_score(X)
            << " O:" << game.get_player_score(O) << std::endl;
  ...
  std::cout << "Board evaluation for " << last_player << ": " << evaluation
            << std::endl;
}
...
void test_line(const std::string &line) {
  ...
  const Matrix<PatternCellData> &line_mat =
      reconizer.get_pattern_cell_matrix(PatternDirection::LeftToRight);
  ...
  for (int col = 0; col < line_mat.get_width(); ++col) {
    ...
    char cell_state =
        (col > 0 && col <= int(line.size())) ? line[col - 1] : 'X';

    auto cell_pattern = reconizer.get_structure_at(
        GomokuCellIndex(0, col - 1), PatternDirection::LeftToRight);

    std::cout << cell_state << " -> " << cell_data << " " << all_structures
              << " " << cell_pattern.first << "["
              << int(cell_pattern.second.col) << "]" << std::endl;
  }
}

int main(int argc, char *argv[]) {
  // If no arguments are given, run the test_problems function
  // If an argument is given, run the test_problem function with the given
  // argument
  if (argc == 1) {
    test_problems();
    return 0;

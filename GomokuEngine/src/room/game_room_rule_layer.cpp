...
static GameActionResult make_action_result(bool success,
...
GameActionResult
GameRoom::GameRuleLayerStandard::perform_action_move(PlayerId player, int row,
                                                     int col) {
  if (_room._game.is_game_over()) {
    return make_action_result(
        false, "The game is over, why are you even still here ?");
  }
...
GameActionResult
GameRoom::GameRuleLayerStandard::perform_action_swap(PlayerId player,
                                                     bool do_the_swap) {
...
GameActionResult GameRoom::GameRuleLayerStandard::perform_pending_action() {
  ...
  if (ai == nullptr) {
    return make_action_result(false, "No pending actions: Player " +
                                         std::to_string(player_id) +
                                         " is not an AI");
  }

  if (_room._action_index == -1) {
    return perform_action_move(player_id, _room._game.get_board_height() / 2,
                               _room._game.get_board_width() / 2);
  }
...
GameActionResult
GameRoom::GameRuleLayerPro::perform_action_move(PlayerId player, int row,
                                                int col) {
  ...
  const bool stone_outside = abs(row - midh) > inner_square_radius ||
                             abs(col - midw) > inner_square_radius;
  ...
  case 1:
    if (!stone_outside) {
      ...
      return make_action_result(false, "Stone should be outside the inner " +
                                           std::to_string(diam) + "x" +
                                           std::to_string(diam) + " square");
    }
    break;
  default:
    break;
...
GameActionResult GameRoom::GameRuleLayerPro::perform_pending_action() {
  if (_room._action_index == -1) {
    return perform_action_move(expected_player(),
                               _room._game.get_board_height() / 2,
                               _room._game.get_board_width() / 2);
  }

  if (_room._action_index == 1 && _room.get_player_ai(expected_player())) {
    return perform_action_move(expected_player(),
                               _room._game.get_board_height() / 2 + 1 +
                                   inner_square_radius,
                               _room._game.get_board_width() / 2);
  }
...
GameActionResult
GameRoom::GameRuleLayerSwap::perform_action_move(PlayerId player, int row,
                                                 int col) {
  if (_room._game.is_game_over()) {
    return make_action_result(
        false, "The game is over, why are you even still here ?");
...
GameActionResult
GameRoom::GameRuleLayerSwap::perform_action_swap(PlayerId player,
                                                 bool do_the_swap) {
  if (!is_swap_expected()) {
...
GameActionResult GameRoom::GameRuleLayerSwap::perform_pending_action() {
  ...
  if (player_id != GameRuleLayerStandard::expected_player()) {
    MoveEvaluation evaluation =
        ai->suggest_move(_room._game, _room._game.get_current_player());
    std::pair<int, int> best_move = getBestMove(evaluation);
    return perform_action_move(player_id, best_move.first, best_move.second);
  }

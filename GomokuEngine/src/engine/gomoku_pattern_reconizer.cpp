...
StructureType PatternCellData::get_relevant_structure() const {
  if (is_gap_open_three) {
    return is_gap_open_three_closed ? StructureType::THREE
                                    : StructureType::OPEN_THREE;
  }
...
void PatternCellData::get_structures_type_count(std::vector<int> &array,
                                                int factor) const {
...
PatternCellIndex::PatternCellIndex(int row, int col)
    : Matrix<PatternCellData>::Index(row, col) {}

PatternCellIndex::PatternCellIndex(GomokuCellIndex gomoku_index)
    : Matrix<PatternCellData>::Index(gomoku_index.row + 1,
                                     gomoku_index.col + 1) {}
...
GomokuPatternReconizer::GomokuPatternReconizer(Player player)
    : _gomoku_player(player), _cell_matrices(static_cast<int>(
                                  PatternDirection::Count_PatternDirection)),
      _structure_maps(
          static_cast<int>(PatternDirection::Count_PatternDirection)),
      _cached_pattern_count(
          static_cast<int>(StructureType::COUNT_STRUCTURE_TYPE), 0),
      _tagging_mode(false) {}

GomokuPatternReconizer::GomokuPatternReconizer(
    const GomokuPatternReconizer &copy)
    : _gomoku_player(copy._gomoku_player), _cell_matrices(copy._cell_matrices),
...
void GomokuPatternReconizer::update_patterns_with_move(
    const GomokuGame &board, const MoveResult &Count_move) {
  TIMER
  for (CellChange change : Count_move.cell_changes) {
    update_cell(board,
                PatternCellIndex(GomokuCellIndex(change.row, change.col)));
  }
}
...
void GomokuPatternReconizer::print_patterns() {
  ...
  for_each_tagged_structures(
      [](PatternCellIndex index, const PatternCellData &data,
         PatternDirection direction, bool &should_continue) {
        GomokuCellIndex game_index = index.to_game_index();
        std::cout << "  " << direction << " " << data << " ["
                  << int(game_index.row) << ';' << int(game_index.col) << "]"
                  << std::endl;
      });

  std::cout << "  cached:" << std::endl;
  for (size_t i = 0; i < _cached_pattern_count.size(); i++) {
    std::cout << "    " << StructureType(i) << ':' << _cached_pattern_count[i]
              << std::endl;
  }
}
...
bool GomokuPatternReconizer::five_or_more_cant_be_captured(
    const GomokuGame &board) {
  bool capturable = false;

  for_each_tagged_structures(
      [this, &capturable,
       board](PatternCellIndex index, const PatternCellData &data,
              PatternDirection direction, bool &should_continue) {
        if (data.structure_length >= 5) {
          for (uint8_t i = 1; i <= data.structure_length; ++i) {
            PatternCellIndex struct_index =
                get_index_offset(index, direction, -i);

            for (int dir = 0; dir < PatternDirection::Count_PatternDirection;
                 ++dir) {
              if (dir == direction)
                continue;

              auto structure = get_structure_at(struct_index.to_game_index(),
                                                PatternDirection(dir));

              const PatternCellData &cell_data =
                  _cell_matrices[dir][PatternCellIndex(structure.second)];
              if (is_structure_capturable(board, structure.second, cell_data,
                                          PatternDirection(dir))) {
                capturable = true;
...
std::pair<StructureType, GomokuCellIndex>
GomokuPatternReconizer::get_structure_at(GomokuCellIndex index,
                                         PatternDirection direction) const {
  const Matrix<PatternCellData> &cell_matrix(_cell_matrices[direction]);

  std::function<std::pair<StructureType, GomokuCellIndex>(PatternCellIndex,
                                                          bool, bool)>
      find_structure;

  find_structure =
      [cell_matrix, &find_structure,
       direction](PatternCellIndex i, bool try_next,
                  bool met_gap) -> std::pair<StructureType, GomokuCellIndex> {
    ...
    if (cell_data.structure_length > 0) {
      ...
      if (!met_gap &&
          (cell_data.structure_length == 1 ||
           cell_data.structure_length == 2) &&
          (!cell_data.is_gap_open_three)) {
        const std::pair<StructureType, GomokuCellIndex> next_structure =
            find_structure(next, false, true);

        return next_structure.first != StructureType::NONE
                   ? next_structure
                   : std::make_pair(cell_data.get_relevant_structure(),
                                    i.to_game_index());
      }

      return std::make_pair(cell_data.get_relevant_structure(),
                            i.to_game_index());
    }
...
const Matrix<PatternCellData> &GomokuPatternReconizer::get_pattern_cell_matrix(
    PatternDirection direction) const {
  return _cell_matrices[direction];
}

PatternCellState
GomokuPatternReconizer::cell_state_at(const GomokuGame &board,
                                      PatternCellIndex index) const {
...
PatternCellData
GomokuPatternReconizer::cell_data_following(const PatternCellData &cell,
                                            PatternCellState state) const {
  ...
  case Blocked:
    ...
    const bool structure_present =
        (!cell.is_sequence_closed && cell.sequence_length > 0) ||
        cell.sequence_length >= 5;
    ...
    result.is_gap_open_three =
        ...
        ((cell.previous_structure_length == 2 &&
          result.structure_length == 1) ||
         (cell.previous_structure_length == 1 && result.structure_length == 2));
...
const PatternCellData &GomokuPatternReconizer::cell_data_following_memoized(
    const PatternCellData &cell, PatternCellState state) const {
        ...
        for (int seq_closed = 0; seq_closed < 2; ++seq_closed) {
          for (int prev_len = 0; prev_len < max_prev_len; ++prev_len) {
            for (int prev_closed = 0; prev_closed < 2; ++prev_closed) {
              ...
              cell_values[ss][seq_len][seq_closed][prev_len][prev_closed] =
                  current;
            }
          }
        }
  ...
  return cell_values[state][cell.sequence_length]
                    // [cell.sequence_length >= max_seq_len ? max_seq_len :
                    // cell.sequence_length]
                    [cell.is_sequence_closed]
                    // [cell.previous_structure_length >= max_prev_len ?
                    // max_prev_len : cell.previous_structure_length]
...
bool GomokuPatternReconizer::adjust_matrices_size(const GomokuGame &board) {
  ...
  for (int i = 0; i < PatternDirection::Count_PatternDirection; ++i) {
    ...
    const bool size_differ = _cell_matrices[i].get_width() != width ||
                             _cell_matrices[i].get_height() != height;
...
void GomokuPatternReconizer::update_cell(const GomokuGame &board,
...
void GomokuPatternReconizer::update_cell_direction(const GomokuGame &board,
                                                   PatternCellIndex index,
...
std::pair<int, int>
GomokuPatternReconizer::get_direction_offset(PatternDirection direction) {
  switch (direction) {
  case PatternDirection::LeftToRight:
...
PatternCellIndex GomokuPatternReconizer::get_index_offset(
    PatternCellIndex index, PatternDirection direction, int distance) {
  switch (direction) {
  case PatternDirection::LeftToRight:
...
void GomokuPatternReconizer::activate_tagging_mode() {
  ...
  for (int dir = 0; dir < PatternDirection::Count_PatternDirection; ++dir) {
    ...
    for (index.row = 0; index.row < _cell_matrices[dir].get_height();
         ++index.row) {
      for (index.col = 0; index.col < _cell_matrices[dir].get_width();
           ++index.col) {
...
bool GomokuPatternReconizer::is_relevant_to_tag(
    const PatternCellData &data) const {
  if (data.structure_length >= 5)
    return true;
...
void GomokuPatternReconizer::untag_celldata_structure(
    PatternCellIndex index, PatternDirection direction) {
  if (_tagging_mode)
    _structure_maps[direction][index.row].erase(index.col);
}

void GomokuPatternReconizer::tag_celldata_structure(
    PatternCellIndex index, PatternDirection direction) {
  if (_tagging_mode)
    _structure_maps[direction][index.row].insert(index.col);
}

void GomokuPatternReconizer::for_each_tagged_structures(
    std::function<void(PatternCellIndex, const PatternCellData &,
                       PatternDirection, bool &should_continue)>
        lambda) {
  activate_tagging_mode();
  bool should_continue = true;
  for (int direction = 0; direction < PatternDirection::Count_PatternDirection;
       ++direction) {
...
bool GomokuPatternReconizer::is_structure_capturable(
    const GomokuGame &board, PatternCellIndex index,
    const PatternCellData &data, PatternDirection direction) const {
  if (data.structure_length == 2 && data.is_structure_closed) {
    const GomokuCellIndex game_index = index.to_game_index();
    const bool index_is_valid =
        board.coordinates_are_valid(game_index.row, game_index.col);
    if (index_is_valid) {
      const GomokuCellIndex opposite_side_index =
          get_index_offset(index, direction, -3).to_game_index();
      const bool opposite_side_is_valid = board.coordinates_are_valid(
          opposite_side_index.row, opposite_side_index.col);

...
void writeMoveEvaluation(std::ostream &out, const MoveEvaluation &eval,
                         int depth) {
  // Create indentation based on the depth
  std::string indent(depth * 4, ' '); // 4 spaces for each level of depth

  out << indent << "Move: " << coordinate_to_char(eval.move.first)
      << coordinate_to_char(eval.move.second) << " | " << eval.score << "\n";
  if (depth < get_depth_from_env()) {
    out << indent << "Evals: " << eval.neededEvalCount << " / "
        << eval.evaluatedMoves << " / " << eval.totalEvalCount << "\n";
  }
...
void writeSurplusEvaluation(std::ofstream &out, const MoveEvaluation &eval,
                            int depth) {
  if (depth == get_depth_from_env()) {
    return;
  }
  ...
  if (eval.neededEvalCount != eval.evaluatedMoves) {
    out << eval.neededEvalCount << " / " << eval.evaluatedMoves << " / "
        << eval.totalEvalCount << "\n";
  }
...
std::pair<int, int> getBestMove(const MoveEvaluation &eval,

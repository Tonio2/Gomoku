...
GomokuAI::GomokuAI(int depth, GomokuAIData data)
    : game(0, 0), depth(depth), evaluation_data(data), move_count(0),
...
void GomokuAI::sortMoves(std::vector<MoveHeuristic> &moves,
  ...
  for (MoveHeuristic &move : moves) {
    try {
      ...
      move.score = maximizingPlayer ? std::numeric_limits<int>::min() + 1
...
MoveEvaluation GomokuAI::minimax(int depth, int alpha, int beta,
                                 bool maximizingPlayer, int row, int col) {
  TIMER
  ...
  // Else find all the relevant moves and sort them by their heuristic
  // evaluation if the depth is not 1.
  ...
  // For each move, make the move, call minimax recursively and reverse the
  // move.
  int extremeEval = maximizingPlayer ? std::numeric_limits<int>::min()
                                     : std::numeric_limits<int>::max();
  for (const MoveHeuristic &move : moves) {
    try {
      MoveResult game_move = game.make_move(move.row, move.col);
      MoveEvaluation evalNode = minimax(depth - 1, alpha, beta,
                                        !maximizingPlayer, move.row, move.col);
      game.reverse_move(game_move);
...
int GomokuAI::score_player(Player player) {
  ...
  score += (game.get_patterns_count(player)[OPEN_THREE] +
                game.get_patterns_count(player)[FOUR] +
                game.get_patterns_count(player)[OPEN_FOUR] >=
            2)
               ? evaluation_data.value_of_multiple_forced()
               : 0;
  score += (game.get_patterns_count(player)[OPEN_FOUR] >= 2
                ? evaluation_data.value_of_multiple_o4()
                : 0);
  score += evaluation_data.value_of_captures(game.get_player_score(player));
  return score;
}

int GomokuAI::heuristic_evaluation(const GomokuGame &board,
                                   Player heuristic_player) {
  TIMER
  return score_player(heuristic_player) -
         score_player(board.other_player(heuristic_player));
}
...
MoveEvaluation GomokuAI::suggest_move(const GomokuGame &board, Player player) {
  ...
  MoveEvaluation result =
      minimax(depth, std::numeric_limits<int>::min(),
              std::numeric_limits<int>::max(), true, -1, -1);
  return result;
}
...
std::vector<MoveHeuristic> GomokuAI::find_relevant_moves() const {
  ...
  for (int row = 0; row < game.get_board_height(); ++row) {
    for (int col = 0; col < game.get_board_width(); ++col) {
      if (game.get_board_value(row, col) == E) { // Empty cell
        ...
        for (const auto &dir : directions) {
          for (int step = 1; step <= 2; ++step) {
            ...
            if (game.coordinates_are_valid(newRow, newCol) &&
                game.get_board_value(newRow, newCol) != E) {
              relevantMoves.push_back(
                  MoveHeuristic{uint8_t(row), uint8_t(col), 0});
              foundStone = true;
              break; // No need to check further if one stone is found near this
                     // cell.
            }
          }
          if (foundStone) {

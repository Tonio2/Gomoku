...
std::pair<int, int> GomokuAI::suggest_move()
{
    auto result = minimax(depth, std::numeric_limits<int>::min(), std::numeric_limits<int>::max(), true);
    std::cout << "Best move is " << result.second.first << ", " << result.second.second << std::endl
              << "With a score of " << result.first << std::endl;
    return result.second; // Return the best move
}

            ...
            with open(filepath, "r") as file:
                ...
                for line_num, line in enumerate(lines, start=1):
                    if "Stats: Best:" in line:
                        match = re.search(
                            r"Best:\s*(\d+)\s*\|\s*Evaluated:\s*(\d+)\s*\|\s*Total:\s*(\d+)",
                            line,
                        )
                        if match:
            ...
            with open(filepath, "r") as file:
                ...
                if "Stats: Best:" in line:
                    ...
                    if match:
                        best = int(match.group(1))
                        total = int(match.group(3))
                        if total > 0 and (best / total) > 0.80:
                            results.append((filename, 1, line.strip()))
    return results


def main():
    directory = "./GomokuBack/move_evaluation"  # Change this to the path where your files are located if necessary
